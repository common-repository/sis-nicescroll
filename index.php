...
Description: Nicescroll is a jquery plugin for WordPress site.
Version: 1.2
Author: sayful
Author URI: http://sayful.net
Text Domain: nicescroll
...
function sis_nicescroll_add_menu(){
   add_options_page( __('Nicescroll', 'nicescroll'), __('Nicescroll', 'nicescroll'), 'manage_options', 'sis-nicescroll/plugin-options.php');
}
...
function sis_nicescroll_validate_data($input) {
  ...
  $options['cursor_width_hover'] = wp_filter_nohtml_kses( $input['cursor_width_hover'] );
...
jQuery(document).ready(function() {  
    ...
    jQuery('.nicescroll-rails').mouseover(function(){
      jQuery('.nicescroll-rails div').css("width","<?php echo $options['cursor_width_hover']; ?>");
    });

    jQuery('.nicescroll-rails').mouseout(function(){
      jQuery('.nicescroll-rails div').css("width","<?php echo $options['cursor_width']; ?>");
    });
});
</script>
<?php }
